...
def randomWordsFromFile(file, quant):
    ...
    # ensure that # words to be used in wordsearch <= wordListLength
...
def checkCoords(matrix, rowIndex, colIndex, word, x):
    ...
    # check to see if row or column indices have walked off of the matrix in any direction
...
# checks if word can fit in matrix at provided position.
...
def plotWords(words, matrix):
    ...
    for word in words:
        ...
        while not plottable:
            ...
            plottableAngles = []
            # get random position/matrix indices
...
# generate initial blank matrix and print it out
matrix = generateMatrix(matrixRows, matrixCols)
for index, row in enumerate(matrix):
    print(index,row)


# generate words from word list
words = randomWordsFromFile("words.txt", numWords)


# print out matrix generated by plotwords function
for index, row in enumerate(plotWords(words, matrix)):
    # index is the number of the rows (from enumeration), row is the actual list.
    print(index, row)
print(words)


# print final word search
for row in replaceBlanks(matrix):
    print(row)
print(words)
...
# Try each indice until all have been tried (use hash table or equivalent to keep track??)
